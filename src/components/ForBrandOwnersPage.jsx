...
function ListYourBrandHeroSection() {
  return (
    <section
      id="lyb-hero-section"
      className={`lyb-hero-section ${LYB_SECTION} flex w-full flex-col justify-center overflow-hidden py-8 sm:py-10 lg:py-12`}
      style={{ minHeight: HERO_VIEWPORT_H }}
    >
      <div className="relative z-10 w-full">
        <div className="mx-auto w-full max-w-[1280px] px-6 lg:px-10 xl:max-w-[1360px]">
          <div className="grid items-center gap-10 lg:grid-cols-[minmax(0,1fr)_minmax(500px,600px)] lg:gap-10 xl:grid-cols-[minmax(0,0.92fr)_minmax(540px,640px)] xl:gap-12">
            <motion.div
            ...
            <motion.div
              ...
              className="lyb-hero-form-slot w-full"
            >
...
const inputClass =
  'lyb-form-field w-full min-h-[44px] rounded-xl border border-slate-200 bg-white px-3.5 py-2.5 text-sm text-black placeholder:text-slate-400 shadow-sm transition focus:border-violet-500 focus:outline-none focus:ring-2 focus:ring-violet-500/20';
...
function Field({ label, required, children, className = '' }) {
  return (
    <div className={`flex flex-col ${className}`}>
      <label className="lyb-form-label mb-1.5 block text-[0.68rem] font-bold uppercase tracking-wide">
        {label}
        {required && (
...
function HeroBrandInquiryForm({ id = 'hero-brand-inquiry' }) {
  ...
  return (
    <motion.div
      ...
      className="lyb-hero-form relative w-full"
      style={{ willChange: 'transform, opacity' }}
    >
      <motion.div className="lyb-form-panel relative flex w-full flex-col overflow-hidden rounded-3xl border border-violet-500/25 bg-gradient-to-br from-[#1a0f3d] via-[#12082a] to-[#0a0618] p-5 shadow-[0_24px_64px_rgba(0,0,0,0.45)] sm:p-6 lg:p-7">
        <div
        ...
        <div className="relative mb-5 shrink-0 border-b border-white/10 pb-4">
          <p className="lyb-form-eyebrow text-[0.62rem] font-bold uppercase tracking-[0.2em] text-white/90">Brand inquiry</p>
          <h2 className={`lyb-form-title mt-1 ${TYPE.formTitle} text-white`}>Start Your Listing</h2>
        </div>
